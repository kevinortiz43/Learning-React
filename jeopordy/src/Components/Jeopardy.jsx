  ...
  return (
    <div className="GameShow-Container">
      {gameShow.map((gameShow, index) => (
        <div key={index}>
          <button className="Get-Question-Button" onClick={() => gameShow2()}>
            {" "}
            Get next question
          </button>
          ...
          <div className="flex">
            <h1>
              Name <Points points={gameShow.value} />
            </h1>
          </div>
          <button
            className="Get-Answer-Button"
            onClick={() => setQuestion(!hideQuestion)}
          >
            Click me to get the answer
          </button>
          {hideQuestion ? (
            <p className="Answer">Answer: {gameShow.answer}</p>
          ) : null}
          <br />
        </div>
      ))}
